...
def make_nn_funs(layer_sizes, L2_reg):
    ...
    def predictions(W_vect, inputs, alpha):
        ...
        return sigmoid(alpha) * outputs + (1 - sigmoid(alpha)) * prev_outputs #np.dot((outputs - b), inv_W) 
...
if __name__ == '__main__':
    ...
    optimize.minimize(loss_fun, W, jac=loss_grad_W, method='L-BFGS-B', \
        args=(train_images, train_labels, alpha), options={'disp': True}, callback=print_perf)
    #optimize.minimize(loss_fun, W, jac=loss_grad_W, method='L-BFGS-B', \
    #    args=(train_images, train_labels), callback=print_perf)
    """
    # Train with sgd
